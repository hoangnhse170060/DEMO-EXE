...
import { getMembershipPlan } from './memberships'
...
const MEMBERSHIP_REWARD_STATUS = 'membership_reward'
...
const applyLocalMembershipReward = async (order: LocalOrder) => {
  const plan = getMembershipPlan(order.product_id)
  if (!plan) return
  const alreadyGranted = order.order_status_history.some((entry) => entry.status === MEMBERSHIP_REWARD_STATUS)
  if (alreadyGranted) return

  const currentUser = localAuth.getCurrentUser()
  if (!currentUser || currentUser.id !== order.user_id) return

  const updatedStars = (currentUser.stars ?? 0) + plan.rewardStars
  await localAuth.updateUserStars(currentUser.id, updatedStars)

  const timestamp = new Date().toISOString()
  appendHistory(order, MEMBERSHIP_REWARD_STATUS, `Đã cộng ${plan.rewardStars} sao cho gói ${plan.label}.`, timestamp)
}

const applySupabaseMembershipReward = async (orderId: string, actorId?: string): Promise<boolean> => {
  try {
    ...
    await supabase
      .from('user_profiles')
      .update({ stars: currentStars + plan.rewardStars })
      .eq('user_id', order.user_id)
    ...
    await supabase
      ...
      .insert({
        order_id: orderId,
        status: MEMBERSHIP_REWARD_STATUS,
        notes: `Đã cộng ${plan.rewardStars} sao cho gói ${plan.label}.`,
        created_by: actorId,
      })

    return true
  } catch (error) {
    console.warn('Không thể cộng sao Supabase cho gói thành viên:', error)
    return false
  }
}
...
export const paymentService = {
  ...
  async initiatePayment(orderId: string, paymentMethodType: string): Promise<PaymentResult> {
    if (!isSupabaseConfigured) {
      ...
      await applyLocalMembershipReward(order)
      writeLocalOrders(orders)
  ...
  async recordPaymentSuccess(orderId: string, _method: string, transactionData: any) {
    ...
    await applySupabaseMembershipReward(orderId, user?.id)
