...
export default function Sidebar() {
  ...
  const navItems: NavItem[] = [
    { id: 'home', label: 'HOME', path: '/' },
    {
      ...
      path: '/history',
      subItems: [
        { label: 'French Era', path: '/history?era=french&phase=antiFrench' },
        { label: 'American Era', path: '/history?era=american&phase=antiAmerican' },
      ],
    },
  ...
  return (
    <>
      {!isAdminDashboard && !isEventDetailPage && (
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="fixed top-6 left-3 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-white/80 text-charcoal-900 shadow-brand transition hover:bg-white"
